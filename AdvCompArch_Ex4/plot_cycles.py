...
for dirname in sys.argv[1:]:
	...
	(cycles, version) = get_cycles_from_output_file(output_file1)
	results_tuples.append((version, threads, cycles))
	
	(cycles, version) = get_cycles_from_output_file(output_file2)
	results_tuples.append((version, threads, cycles))
	
	(cycles, version) = get_cycles_from_output_file(output_file3)
	results_tuples.append((version, threads, cycles))
	
	(cycles, version) = get_cycles_from_output_file(output_file4)
	results_tuples.append((version, threads, cycles))
	
	(cycles, version) = get_cycles_from_output_file(output_file5)
	results_tuples.append((version, threads, cycles))
...
ax.set_ylabel("$Number of Cycles(x10^6)$")
...
plt.savefig("Grain_Size-"+grain+'.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
