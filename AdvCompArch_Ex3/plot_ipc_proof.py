...
def get_params_from_basename(basename):
	...
	bench = tokens[0]
	input_size = 'ref'
	ws = int(tokens[1].split('-')[0].split('_')[1])
	dw = int(tokens[1].split('-')[1].split('_')[1])
...
for tuple in tuples_by_dw:
	...
	ws_axis = tuple[1][0]
	ipc_axis = tuple[1][1]
	x_ticks = np.arange(0, len(global_dw))
	x_labels = map(str, global_dw)
	ax.xaxis.set_ticks(x_ticks)
	ax.xaxis.set_ticklabels(x_labels)

	print x_ticks
	print ipc_axis
	ax.plot(x_ticks, ipc_axis, label="WS_"+str(dw), marker=markers[i%len(markers)])
	i = i + 1
plt.title(bench +": IPC testing")

lgd = ax.legend(ncol=len(tuples_by_dw), bbox_to_anchor=(0.9, -0.1), prop={'size':8})
plt.savefig(bench+'-'+'proof.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
